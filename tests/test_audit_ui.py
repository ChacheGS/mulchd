...
def _jot(tmp_path: Path, domain: str, **fields) -> dict:
    ...
    record = {"id": f"mx-{uuid.uuid4().hex[:8]}", "recorded_at": datetime.now(timezone.utc).isoformat(), **fields}
...
async def test_audit_page_with_project_shows_events(admin_client, tmp_path, monkeypatch):
    ...
    r = _jot(tmp_path, "api", type="convention", classification="tactical",
             content="Keep API keys out of logs", owner="alice")
    await RecordMeta.create(record_id=r["id"], project=project, domain="api",
                            author=alice, session_id=uuid.uuid4(), client="test")
    await RecordEvent.create(record_id=r["id"], project=project, domain="api",
                             actor=alice, action="write", client="test", session_id=uuid.uuid4())
...
async def test_audit_cross_owner_edit_shows_badge(admin_client, tmp_path, monkeypatch):
    ...
    org, project, alice, bob = await _setup(tmp_path, monkeypatch)
    r = _jot(tmp_path, "api", type="convention", classification="tactical",
             content="Original rule", owner="alice")
    await RecordMeta.create(record_id=r["id"], project=project, domain="api",
                            author=alice, session_id=uuid.uuid4(), client="test")
    sid = uuid.uuid4()
    await RecordEvent.create(record_id=r["id"], project=project, domain="api",
                             actor=bob, action="edit", client="test", session_id=sid)
    await RecordEdit.create(record_id=r["id"], project=project, domain="api",
                            actor=bob, before_snapshot={"content": "Original rule"},
                            client="test", session_id=sid)
...
async def test_audit_same_owner_edit_no_badge(admin_client, tmp_path, monkeypatch):
    ...
    org, project, alice, bob = await _setup(tmp_path, monkeypatch)
    r = _jot(tmp_path, "api", type="convention", classification="tactical",
             content="Alice's rule", owner="alice")
    await RecordMeta.create(record_id=r["id"], project=project, domain="api",
                            author=alice, session_id=uuid.uuid4(), client="test")
    sid = uuid.uuid4()
    await RecordEvent.create(record_id=r["id"], project=project, domain="api",
                             actor=alice, action="edit", client="test", session_id=sid)
    await RecordEdit.create(record_id=r["id"], project=project, domain="api",
                            actor=alice, before_snapshot={"content": "Alice's rule"},
                            client="test", session_id=sid)
...
async def test_audit_cross_owner_falls_back_to_jsonl_owner(admin_client, tmp_path, monkeypatch):
    ...
    org, project, alice, bob = await _setup(tmp_path, monkeypatch)
    r = _jot(tmp_path, "api", type="convention", classification="tactical",
             content="Old rule", owner="alice")
    # No RecordMeta row — simulates a record pre-dating that table
    sid = uuid.uuid4()
    await RecordEvent.create(record_id=r["id"], project=project, domain="api",
                             actor=bob, action="edit", client="test", session_id=sid)
    await RecordEdit.create(record_id=r["id"], project=project, domain="api",
                            actor=bob, before_snapshot={"content": "Old rule"},
                            client="test", session_id=sid)
...
async def test_audit_write_superseding_foundational_with_lower_tier(admin_client, tmp_path, monkeypatch):
    """Write that supersedes a foundational record with a lower tier shows 'foundational → tactical'."""
    org, project, alice, bob = await _setup(tmp_path, monkeypatch)
    original = _jot(tmp_path, "api", type="convention", classification="foundational",
                    content="Guardrail", owner="alice")
    new_rec = _jot(tmp_path, "api", type="convention", classification="tactical",
                   content="Weakened", owner="bob", supersedes=[original["id"]])
    await RecordEvent.create(record_id=new_rec["id"], project=project, domain="api",
                             actor=bob, action="write", client="test", session_id=uuid.uuid4())
...
async def test_audit_write_superseding_foundational_same_tier(admin_client, tmp_path, monkeypatch):
    ...
    original = _jot(tmp_path, "api", type="convention", classification="foundational",
                    content="Guardrail", owner="alice")
    new_rec = _jot(tmp_path, "api", type="convention", classification="foundational",
                   content="Replacement guardrail", owner="bob", supersedes=[original["id"]])
    await RecordEvent.create(record_id=new_rec["id"], project=project, domain="api",
                             actor=bob, action="write", client="test", session_id=uuid.uuid4())
...
async def test_audit_edit_that_lowers_classification(admin_client, tmp_path, monkeypatch):
    ...
    org, project, alice, bob = await _setup(tmp_path, monkeypatch)
    r = _jot(tmp_path, "api", type="convention", classification="tactical",
             content="Some rule", owner="alice")
    sid = uuid.uuid4()
    await RecordEvent.create(record_id=r["id"], project=project, domain="api",
                             actor=alice, action="edit", client="test", session_id=sid)
    # before_snapshot shows it was foundational before the edit
    await RecordEdit.create(record_id=r["id"], project=project, domain="api",
                            actor=alice, before_snapshot={"classification": "foundational"},
                            client="test", session_id=sid)
...
async def test_audit_tactical_supersedes_tactical_no_badge(admin_client, tmp_path, monkeypatch):
    ...
    org, project, alice, bob = await _setup(tmp_path, monkeypatch)
    original = _jot(tmp_path, "api", type="convention", classification="tactical",
                    content="Old approach", owner="alice")
    new_rec = _jot(tmp_path, "api", type="convention", classification="tactical",
                   content="New approach", owner="bob", supersedes=[original["id"]])
    await RecordEvent.create(record_id=new_rec["id"], project=project, domain="api",
                             actor=bob, action="write", client="test", session_id=uuid.uuid4())
