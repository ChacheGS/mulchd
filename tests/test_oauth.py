...
def test_github_provider_configured(monkeypatch):
    ...
    assert (
        ProviderInfo(key="github", display_name="GitHub", logo_url=None)
        in get_configured_providers()
    )
...
def test_oidc_provider_missing_client_secret_is_skipped(monkeypatch):
    ...
    monkeypatch.setenv(
        "MULCHD_OIDC_BROKEN_DISCOVERY_URL",
        "https://broken.example.com/.well-known/openid-configuration",
    )
